...
public class MangaSettings
{
	...
	public MangaSettings( File propfile_in )
	{
		...
		if( properties == null )
		{
			...
			Log.warn( "Settings file " + propfile + " was not present, so it was generated by default." );
		}
	...
	private void setDefaultProperties()
	{
		...
		Log.fatal( "No " + propfile.getName() + " detected.  Please set the settings in this file and rerun the program." );
	}
	...
	private void checkKey( String key )
	{
		...
		if( value == null )
		{
			if( isPropertyRequired( key ) )
			{
				Log.fatal( key + " is required, but not present" );
			}
		}
	}
	...
	private String invalidProperty( String key, boolean fatal )
	{
		...
		{
			Log.fatal( "No default value for " + key );
		}
		
		if( !fatal )
		{
			Log.fatal( "Setting for " + key + " invalid, using default of '" + default_value + "'" );
		}
		else
		{
			Log.fatal( "Setting for " + key + " is invalid and required" );
			System.exit( 0 );
		}
	...
	private void checkSettings()
	{
		for( String[] strings : PROPERTIES_SCHEMA )
		{
			...
			if( properties.get( key ) == null )
			{
				if( isPropertyRequired( key ) )
				{
					Log.fatal( key + " is required and not present" );
					System.exit( 0 );
				}
			}
		}
